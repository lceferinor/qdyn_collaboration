...
% This 2 are redifined afterwards
...
p.THETA_LAW=1; % Edited on 06/21 (Is this better?)
...
p.DC=0.1; % It was very small, Tohoku is 0.8
...
%p.V2=0.01; % 1) Should we change this? Would this be reasonable for the 
           % Peru subduction zone? (Luis). RNS is 0, so we do not need to
           % define this
...
dc_asp=0.025;       %Dc of asperity (?????????????????????? Luis)
