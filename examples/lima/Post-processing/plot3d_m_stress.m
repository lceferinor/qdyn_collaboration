...
% for i=1:10
  ...
  tau = ox1.sigma(:,i)'.*(p.MU_SS+p.A(ind_sel).*log(ox1.v(:,i)'/p.V_SS)+p.B(ind_sel).*log(p.V_SS*ox1.th(:,i)'./p.DC(ind_sel)));
  max(tau/1E6)
  scatter3(p.X(ind_sel)/1000+dx/2*p.NXOUT,p.Y(ind_sel)/1000+dy/2*p.NWOUT,p.Z(ind_sel)/1000+dz/2*p.NWOUT,[],tau/1E6,'s','filled');
  caxis([55 65]);
