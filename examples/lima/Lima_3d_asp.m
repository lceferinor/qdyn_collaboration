...
n_asperities = 4;
asp_center_x = [0.128 0.340*0.95 0.431*0.80 0.95*0.622]*790*10^3; % (Scaled on the previous rupture size)
asp_center_zy = [0.735*0.75 0.296*0.8 0.875*1.15 0.315*1.1]*p.W; % From top down
factor_a = 0.8;
factor_b = 0.8;
asp_elip_a = [0.214 0.335*1 0.294*1.4 0.243*1.4]/2*790*10^3*factor_a;% Along strike (Scaled on the previous rupture size)
asp_elip_b = [0.798*1.25 0.667 0.649 0.699*1.1]/2*p.W*factor_b;% Along dip

% n_asperities = 3;
% asp_center_x = [0.128 0.431 0.622]*790*10^3; % (Scaled on the previous rupture size)
% asp_center_zy = [0.735*0.7 0.875*1.2 0.315*1.4]*p.W; % From top down
% factor_a = 0.8;
% factor_b = 0.8;
% asp_elip_a = [0.214 0.294*1.3 0.243]/2*790*10^3*factor_a;% Along strike (Scaled on the previous rupture size)
% asp_elip_b = [0.798 0.649 0.699]/2*p.W*factor_b;% Along dip
...
twm=20000/40;         %warmup time in years (what is this? (Luis))
